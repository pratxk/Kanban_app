...
import { adminGetBoards, adminGetTasks, adminUpdateTaskStatus } from '../redux/admin-redux/action';
...
const AdminTasks = () => {
    ...
    const { boards } = useSelector(state => state.admin);
    const { tasks, totalPages, currentPage } = useSelector(state => state.admin);

    // Fetch boards when the component mounts
    useEffect(() => {
        dispatch(adminGetBoards());
    }, [dispatch]);
    ...
    const onDrop = (taskId, newStatus) => {
        dispatch(adminUpdateTaskStatus(taskId, newStatus));
    };


    useEffect(() => {
        if (selectedBoard) {
            dispatch(adminGetTasks(selectedBoard, page));
        }
